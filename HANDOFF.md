...
Desktop側のエイリアスパスから古いサーバーが起動している可能性が高いです。

対処:

1. `lsof -nP -iTCP:3000 -sTCP:LISTEN` でPIDを確認
2. 古いローカル開発サーバーを停止
3. iCloud Drive実体パスから `npm start`
...
1. 正しい作業ディレクトリから `npm start` する
2. `http://localhost:3000/` が `200 OK` でPWAを返すことを確認する
3. PWA画面で参照画像をドラッグ＆ドロップできるか確認する
4. 画像生成を実行する前に、モデル `gpt-image-2`、サイズ `1024x1536`、参照画像枚数を確認する
5. ユーザーの明示許可後にのみ生成ボタンを押す
